...
**Open O1 — turn reconstruction from claude code parent chain**: how robust is the algorithm against `parentUuid` referring to events not yet ingested (out-of-order delivery)? Need to spike against a 24 MB transcript to confirm linear-scan + buffer works.

...
git add pnpm-workspace.yaml .nvmrc tsconfig.base.json package.json .gitignore pnpm-lock.yaml
git commit -m "chore: bootstrap pnpm workspace and turbo scaffolding"
```

> The `package-lock.json` is an npm lockfile that becomes stale once the repo switches to pnpm. `pnpm-lock.yaml` is generated by `pnpm install` in Step 6; commit it so CI can use `pnpm install --frozen-lockfile` for reproducible builds.

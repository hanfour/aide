...
function translateMessage(msg: OpenAIChatMessage): AnthropicMessage {
  ...
  if (msg.role === 'assistant' && msg.tool_calls !== undefined && msg.tool_calls.length > 0) {
    ...
    const blocks: AnthropicToolUseBlock[] = msg.tool_calls.map(translateToolCall)
    return { role: 'assistant', content: blocks }
  }

  if (msg.role === 'assistant') {
    const content = msg.content ?? ''
    return { role: 'assistant', content: typeof content === 'string' ? content : translateContentParts(content) }
  }
...
function translateContentParts(parts: OpenAIContentPart[]): AnthropicContentBlock[] {
  return parts.map(translateContentPart)
}

function translateContentPart(part: OpenAIContentPart): AnthropicTextBlock | AnthropicImageBlock {
  if (part.type === 'text') {

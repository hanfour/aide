...
describe("expandScope", () => {
  ...
  it("organization scope with null scopeId returns empty sets", () => {
    const exp = expandScope(
      { role: "org_admin", scopeType: "organization", scopeId: null },
      corpus,
    );
    expect(exp.orgs.size).toBe(0);
    expect(exp.depts.size).toBe(0);
    expect(exp.teams.size).toBe(0);
  });

  it("department scope with null scopeId returns empty sets", () => {
    const exp = expandScope(
      { role: "dept_manager", scopeType: "department", scopeId: null },
      corpus,
    );
    expect(exp.orgs.size).toBe(0);
    expect(exp.depts.size).toBe(0);
  ...
  it("team scope with null scopeId returns empty sets", () => {
    const exp = expandScope(
      { role: "team_manager", scopeType: "team", scopeId: null },
      corpus,
    );
    expect(exp.orgs.size).toBe(0);
    expect(exp.depts.size).toBe(0);

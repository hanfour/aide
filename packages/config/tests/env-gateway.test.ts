...
describe('parseServerEnv — gateway vars', () => {
  ...
  it('rejects CREDENTIAL_ENCRYPTION_KEY of wrong length (63 hex chars) regardless of ENABLE_GATEWAY', () => {
    expect(() =>
      parseServerEnv({ ...validBase, CREDENTIAL_ENCRYPTION_KEY: 'a'.repeat(63) }),
    ).toThrow()
  })

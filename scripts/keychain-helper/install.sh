#!/usr/bin/env bash
#
# Install (or re-install) the aide-keychain-helper LaunchAgent.
#
...
LABEL="com.hanfour.aide.keychain-helper"
LAUNCH_AGENT_DIR="$HOME/Library/LaunchAgents"
PLIST_DEST="$LAUNCH_AGENT_DIR/$LABEL.plist"
LOG_DIR="$HOME/Library/Logs"
AIDE_DIR="$HOME/.aide"
TOKEN_PATH="$AIDE_DIR/keychain.token"
HELPER_PORT="${AIDE_KEYCHAIN_PORT:-47823}"
HELPER_HOST="${AIDE_KEYCHAIN_HOST:-127.0.0.1}"
...
for _ in 1 2 3 4 5; do
  if nc -z "$HELPER_HOST" "$HELPER_PORT" 2>/dev/null; then
    ...
    echo "  log:      $LOG_DIR/aide-keychain-helper.log"
    echo
    echo "Test (host):"
    ...
    echo "  ${AIDE_DIR}/keychain.token:/run/aide-keychain.token:ro"
...
echo "  tail -n 20 $LOG_DIR/aide-keychain-helper.log" >&2
exit 1
